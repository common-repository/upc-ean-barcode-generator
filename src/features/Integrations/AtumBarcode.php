...
class AtumBarcode extends CodeStoreField
{
    const OPTION_VALUE = 'atum_barcode';
    const OPTION_NAME = "Barcode - ATUM Inventory Management for WooCommerce";
    ...
    public function getCodeValue($post)
    {
        ...
        return !empty($record) && isset($record->barcode)
            ? $record->barcode
            : "";
    }
    ...
    public function setCodeValue($post, $value, &$usedMetaKey)
    {
        ...
        if (empty($productRow)) {
            $result = $wpdb->insert("{$wpdb->prefix}atum_product_data", array("product_id" => $postId, "barcode" => $value));
        } else {
            $result = $value === ""
                ? $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}atum_product_data SET barcode = null WHERE product_id = %d", $postId))
                : $wpdb->update("{$wpdb->prefix}atum_product_data", array("barcode" => $value), array("product_id" => $postId));
    ...
    public function unsetCodeValue($codeRow)
    {
        ...
        if (!empty(intval($codeRow->product_id))) {
            ...
            $productRow = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}atum_product_data WHERE barcode = %d;", $codeRow->code ), ARRAY_A );
        }

        if (!empty($productRow)) {
            $result =  $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}atum_product_data SET barcode = null WHERE product_id = %d", $productRow['product_id']));
    ...
    public function codeValueNotUsed($value)
    {
        ...
        $record = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}atum_product_data WHERE barcode = %s;", $value));
    ...
    public function addSearchQueryParams($search, $query, $searchStr)
    {
        ...
        $postIds = $wpdb->get_col("
            ...
            WHERE apd.`product_id` IS NOT NULL
              AND (
                apd.`barcode` LIKE '%".esc_sql($searchStr)."%'
              )
        ");
